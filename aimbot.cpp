...
void AimPlayer::UpdateAnimations(LagRecord* record) {
	...
	bool fake = !bot && c_config::get( )->b[ "correct" ];
...
void AimPlayer::OnNetUpdate(Player* player) {
	bool reset = ( !c_config::get( )->b[ "aimbot_enable" ] || player->m_lifeState( ) == LIFE_DEAD || !player->enemy( g_cl.m_local ) );
...
void AimPlayer::SetupHitboxes(LagRecord* record, bool history) {
	...
	if ( c_config::get( )->m[ "head1" ][ 0 ] )
		m_hitboxes.push_back({ HITBOX_HEAD, HitscanMode::PREFER });

	if ( c_config::get( )->m[ "head1" ][ 1 ] && prefer_head)
		m_hitboxes.push_back({ HITBOX_HEAD, HitscanMode::PREFER });

	if ( c_config::get( )->m[ "head1" ][ 2 ] && !(record->m_mode != Resolver::Modes::RESOLVE_NONE && record->m_mode != Resolver::Modes::RESOLVE_WALK && record->m_mode != Resolver::Modes::RESOLVE_BODY))
		m_hitboxes.push_back({ HITBOX_HEAD, HitscanMode::PREFER });

	if ( c_config::get( )->m[ "head1" ][ 3 ] && !(record->m_pred_flags & FL_ONGROUND))
		m_hitboxes.push_back({ HITBOX_HEAD, HitscanMode::PREFER });
	...
	// prefer, always.
	if ( c_config::get( )->m[ "baim1" ][ 0 ] )
		m_hitboxes.push_back( { HITBOX_BODY, HitscanMode::PREFER } );

	// prefer, lethal.
	if ( c_config::get( )->m[ "baim1" ][ 1 ] )
		m_hitboxes.push_back( { HITBOX_BODY, HitscanMode::LETHAL } );

	// prefer, lethal x2.
	if ( c_config::get( )->m[ "baim1" ][ 2 ] )
		m_hitboxes.push_back( { HITBOX_BODY, HitscanMode::LETHAL2 } );

	// prefer, fake.
	if ( c_config::get( )->m[ "baim1" ][ 3 ] &&
		record->m_mode != Resolver::Modes::RESOLVE_NONE &&
		record->m_mode != Resolver::Modes::RESOLVE_WALK &&
		record->m_mode != Resolver::Modes::RESOLVE_BODY )

		m_hitboxes.push_back( { HITBOX_BODY, HitscanMode::PREFER } );

	// prefer, in air.
	if ( c_config::get( )->m[ "baim1" ][ 4 ] && !( record->m_pred_flags & FL_ONGROUND ) )
		m_hitboxes.push_back( { HITBOX_BODY, HitscanMode::PREFER } );

	// prefer, in air.
	if ( c_config::get( )->m[ "baim1" ][ 5 ] &&
		( m_last_move >= c_config::get( )->i[ "misses" ] ||
			m_unknown_move >= c_config::get( )->i[ "misses" ] ) )
		m_hitboxes.push_back({ HITBOX_BODY, HitscanMode::PREFER });

	bool only{ false };

	// only, always.
	if ( c_config::get( )->m[ "baim2" ][ 0 ] ) {
		only = true;
		m_hitboxes.push_back( { HITBOX_BODY, HitscanMode::PREFER } );
	}

	// only, health.
	if ( c_config::get( )->m[ "baim2" ][ 1 ] && m_player->m_iHealth( ) <= ( int )c_config::get( )->i[ "baim_hp" ] ) {
		only = true;
		m_hitboxes.push_back( { HITBOX_BODY, HitscanMode::PREFER } );
	}

	// only, fake.
	if ( c_config::get( )->m[ "baim2" ][ 2 ] &&
		record->m_mode != Resolver::Modes::RESOLVE_NONE &&
		record->m_mode != Resolver::Modes::RESOLVE_WALK &&
		record->m_mode != Resolver::Modes::RESOLVE_BODY ) {
	...
	// only, in air.
	if ( c_config::get( )->m[ "baim2" ][ 3 ] && !( record->m_pred_flags & FL_ONGROUND ) ) {
		only = true;
		m_hitboxes.push_back( { HITBOX_BODY, HitscanMode::PREFER } );
	}

	// only, in air.
	if ( c_config::get( )->m[ "baim2" ][ 4 ] &&
		( m_last_move >= c_config::get( )->i[ "misses" ] ||
			m_unknown_move >= c_config::get( )->i[ "misses" ] ) ) {
	...
	if ( c_keyhandler::get( )->auto_check( "baim_key" ) ) {
		only = true;
		m_hitboxes.push_back( { HITBOX_BODY, HitscanMode::PREFER } );
	}
	...
	auto hitbox = history ? c_config::get( )->m[ "hitbox_history" ] : c_config::get( )->m[ "hitbox" ];
	bool ignore_limbs = record->m_velocity.length_2d( ) > 71.f && c_config::get( )->b[ "ignore_limbs" ];
...
void Aimbot::think() {
	...
	if ( !c_config::get( )->b[ "aimbot_enable" ] )
		return;
...
void Aimbot::find() {
	...
	if ( g_cl.m_weapon_id == ZEUS && !c_config::get( )->b[ "zeusbot" ] )
		return;
	...
	for (const auto& t : m_targets) {
		...
		if ( c_config::get( )->b[ "lagfix" ] && g_lagcomp.StartPrediction( t ) ) {
	...
	if (best.player && best.record) {
		...
		bool on = c_config::get( )->b[ "hitchance" ] && c_config::get( )->i[ "config_mode" ] == 0;
		...
		if ( can_scope ) {
			// always.
			if ( c_config::get( )->i[ "zoom" ] == 1 ) {
				g_cl.m_cmd->m_buttons |= IN_ATTACK2;
				return;
			}

			// hitchance fail.
			else if ( c_config::get( )->i[ "zoom" ] == 2 && on && !hit ) {
				g_cl.m_cmd->m_buttons |= IN_ATTACK2;
				return;
			}
		}
		...
		if (hit || !on) {
			// right click attack.
			if ( c_config::get( )->i[ "config_mode" ] == 1 && g_cl.m_weapon_id == REVOLVER)
				g_cl.m_cmd->m_buttons |= IN_ATTACK2;
...
bool Aimbot::CheckHitchance(Player* player, const ang_t& angle) {
	...
	CGameTrace tr;
	size_t     total_hits{ }, needed_hits{ (size_t)std::ceil(( c_config::get( )->i[ "hitchance_amount" ] * SEED_MAX) / HITCHANCE_MAX) };
...
bool AimPlayer::SetupHitboxPoints(LagRecord* record, BoneArray* bones, int index, std::vector< vec3_t >& points) {
	...
	// get hitbox scales.
	float scale = c_config::get()->i["hitbox_scale"] / 100.f;

	// big inair fix.
	if (!(record->m_pred_flags) & FL_ONGROUND)
		scale = 0.7f;

	float bscale = c_config::get( )->i[ "body_scale" ] / 100.f;
	...
	if (bbox->m_radius <= 0.f) {
		...
		if (index == HITBOX_R_FOOT || index == HITBOX_L_FOOT) {
			...
			if ( c_config::get( )->m[ "multipoint" ][ 3 ] ) {
	...
	// these hitboxes are capsules.
		...
		if (index == HITBOX_HEAD) {
			...
			if ( c_config::get( )->m[ "multipoint" ][ 0 ] ) {
		...
		// body has 5 points.
			...
			// back.
			if ( c_config::get( )->m[ "multipoint" ][ 2 ] )
		...
		// other stomach/chest hitboxes have 2 points.
			...
			if ( c_config::get( )->m[ "multipoint" ][ 1 ] )
				points.push_back({ center.x, bbox->m_maxs.y - r, center.z });
		}
...
bool AimPlayer::GetBestAimPosition(vec3_t& aim, float& damage, LagRecord* record) {
	...
	if (g_cl.m_weapon_id == ZEUS) {
		...
		if ( c_keyhandler::get( )->auto_check( "damage_override_key" ) ) {
			if ( c_config::get( )->b[ "minimum_damage_override_enable" ] ) {
				pendmg = c_config::get( )->i[ "override_damage_amount" ];
			}		
		}
		else
		{
			dmg = c_config::get( )->i[ "minimal_damage" ];
			if ( c_config::get( )->b[ "minimal_damage_hp" ] )
				dmg = std::ceil( ( dmg / 100.f ) * hp );

			pendmg = c_config::get( )->i[ "penetrate_minimal_damage" ];
			if ( c_config::get( )->b[ "penetrate_minimal_damage_hp" ] )
				pendmg = std::ceil( ( pendmg / 100.f ) * hp );

		}

		pen = c_config::get( )->b[ "penetrate" ];

		// scaling our settings with enemy health.
		if ( dmg >= hp )
			dmg = hp;
...
bool Aimbot::SelectTarget(LagRecord* record, const vec3_t& aim, float damage) {
	...
	// fov check.
	if ( c_config::get( )->b[ "fov" ] ) {
		// if out of fov, retn false.
		if (math::GetFOV(g_cl.m_view_angles, g_cl.m_shoot_pos, aim) > c_config::get( )->i[ "fov_amount" ] )
			return false;
	}

	switch ( c_config::get( )->i[ "selection" ] ) {
...
void Aimbot::apply() {
	...
	if (attack || attack2) {
		...
		// nospread.
		if ( c_config::get( )->b[ "nospread" ] && c_config::get( )->i[ "config_mode" ] == 1 )
			NoSpread();

		// norecoil.
		if ( c_config::get( )->b[ "norecoil" ] )
