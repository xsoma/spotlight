...
void HVH::AntiAimPitch() {
	bool safe = c_config::get( )->i[ "config_mode" ] == 0;
	switch ( m_pitch ) {
	case 1:
		// down.
...
void HVH::GetAntiAimDirection() {
	// edge aa.
	if ( c_config::get( )->b[ "edge" ] && g_cl.m_local->m_vecVelocity( ).length( ) < 320.f ) {
		ang_t ang;
	...
	// lock while standing..
	bool lock = c_config::get( )->b[ "dir_lock" ];
...
void HVH::DoRealAntiAim() {
	...
	if (m_yaw > 0) {
		...
		bool stand = c_config::get( )->i[ "body_fake_stand" ] > 0 && m_mode == AntiAimMode::STAND;
		bool air = c_config::get( )->i[ "body_fake_air" ] > 0 && m_mode == AntiAimMode::AIR;
		...
		if (stand && !g_cl.m_lag && g_csgo.m_globals->m_curtime >= (g_cl.m_body_pred - g_cl.m_anim_frame) && g_csgo.m_globals->m_curtime < g_cl.m_body_pred) {
			// z mode.
			if ( c_config::get( )->i[ "body_fake_stand" ] == 4 )
				g_cl.m_cmd->m_view_angles.y -= 90.f;
		}

		float custom = c_config::get( )->i[ "body_fake_stand_custom" ] * 2.5;
		static int negative = false;

		if (c_config::get()->b["body_fake_stand_fakewalk"] ) {
			// check if we will have a lby fake this tick.
			if ( !g_cl.m_lag && g_csgo.m_globals->m_curtime >= g_cl.m_body_pred && stand && !c_keyhandler::get( )->auto_check( "fakewalk_key" ) ) {
				// there will be an lbyt update on this tick.
				if (stand) {
					switch ( c_config::get( )->i[ "body_fake_stand" ] ) {
					...
					switch ( c_config::get()->i["body_fake_air"] ) {
...
void HVH::DoFakeAntiAim() {
	...
	*g_cl.m_packet = true;
	...
	switch ( c_config::get( )->i[ "fake_yaw" ] ) {
	...
	case 2:
		...
		g_cl.m_cmd->m_view_angles.y += c_config::get( )->i[ "fake_relative" ];
		break;

		// relative jitter.
	case 3: {
		// get fake jitter range from menu.
		float range = c_config::get( )->i[ "fake_jitter_range" ] / 2.f;
...
void HVH::AntiAim() {
	...
	if (m_mode == AntiAimMode::STAND) {
		m_pitch = c_config::get( )->i[ "pitch" ];
		m_yaw = c_config::get( )->i[ "yaw_stand" ];
		m_jitter_range = c_config::get( )->i[ "jitter_range" ];
		m_rot_range = c_config::get( )->i[ "rotate_range" ];
		m_rot_speed = c_config::get( )->i[ "rotate_speed" ];
		m_rand_update = c_config::get( )->i[ "rotate_update" ];
		m_dir = c_config::get( )->i[ "direction" ];
		m_dir_custom = c_config::get( )->i[ "dir_custom" ];
		m_base_angle = c_config::get( )->i[ "base_angle" ];
		m_auto_time = c_config::get( )->i[ "dir_time" ];
	}
	...
	else if ( c_config::get( )->i[ "fake_yaw" ] > 0 )
		m_direction = g_cl.m_cmd->m_view_angles.y;

	if ( c_config::get( )->i[ "fake_yaw" ] ) {
...
void HVH::SendPacket( ) {
	...
	// fake-lag enabled.
	if ( c_config::get( )->b[ "lag_enable" ] && !g_csgo.m_gamerules->m_bFreezePeriod( ) && !( g_cl.m_flags & FL_FROZEN ) ) {
		// limit of lag.
		int limit = std::min( ( int )c_config::get( )->i[ "lag_limit" ], g_cl.m_max_lag );

		int variance = std::clamp( ( int )c_config::get( )->i[ "lag_variance" ], 1, 100 );
		...
		if ( c_config::get( )->b[ "lag_customize" ] )
		{
			// move.
			if ( c_config::get( )->m[ "lag_active" ][ 0 ] && delta > 0.1f && g_cl.m_speed > 0.1f ) {
				active = true;
			}

			// air.
			else if ( c_config::get( )->m[ "lag_active" ][ 1 ] && ( ( g_cl.m_buttons & IN_JUMP ) || !( g_cl.m_flags & FL_ONGROUND ) ) ) {
				active = true;
			}

			// crouch.
			else if ( c_config::get( )->m[ "lag_active" ][ 2 ] && g_cl.m_local->m_bDucking( ) ) {
				active = true;
		...
		if ( active ) {
			int mode = c_config::get( )->i[ "lag_mode" ];
	...
	if ( c_keyhandler::get( )->auto_check( "fakewalk_key" ) ) {
		*g_cl.m_packet = false;
	}

	// do not lag while shooting.
	if ( c_config::get( )->i[ "lag_shoot" ] && g_cl.m_old_shot )
		*g_cl.m_packet = true;


	int entity = g_cl.m_local->index( ) % 32;
	if ( c_config::get( )->b[ "lag_breaker" ] )
	{
		auto tick_count = g_cl.m_cmd->m_tick + 8 - entity;

		auto nBaseTick = 100 * ( tick_count / 100 );
		if ( nBaseTick <= 100 * ( ( tick_count - 1 ) / 100 ) )
		{
			auto v26 = ( g_cl.m_cmd->m_tick ) - entity;
