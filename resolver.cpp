...
void Resolver::MatchShot(AimPlayer* data, LagRecord* record) {
	...
	if ( c_config::get( )->i[ "config_mode" ] == 1 )
		return;
...
void Resolver::SetMode(LagRecord* record) {
	...
	if (g_input.GetKeyState( c_keyhandler::get( )->auto_check( "override_key" )) && record->m_flags & FL_ONGROUND && (speed <= 0.1f || record->m_fake_walk))
		record->m_mode = Modes::RESOLVE_OVERRIDE;   

	// if on ground, not moving or fakewalking.
	else if ((record->m_flags & FL_ONGROUND) && (speed <= 30.0f || record->m_fake_walk) && !g_input.GetKeyState( c_keyhandler::get( )->auto_check( "override_key" )))
...
void Resolver::ResolveAngles(Player* player, LagRecord* record) {
	...
	else if (record->m_mode == Modes::RESOLVE_OVERRIDE || (g_input.GetKeyState( c_keyhandler::get( )->auto_check( "override_key" ) )))
		ResolveOverride(player, record, data);

	else if (record->m_mode == Modes::RESOLVE_LASTMOVE || record->m_mode == Modes::RESOLVE_UNKNOWM && !(g_input.GetKeyState( c_keyhandler::get( )->auto_check( "override_key" ) )))
...
void Resolver::ResolveOverride(Player* player, LagRecord* record, AimPlayer* data) {
	...
	if (g_input.GetKeyState( c_keyhandler::get( )->auto_check( "override_key" ) )) {
		ang_t                          viewangles;
