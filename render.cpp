...
WORD DefineFont(const char* name, int height, int weight, int blur, int scanlines, int flags)
{
	DWORD temp = g_csgo.m_surface->CreateFont();

	g_csgo.m_surface->SetFontGlyphSet(temp, name, height, weight, blur, scanlines, flags);
...
{
	tahoma_8 = DefineFont("Tahoma", 8, 400, 0, 0, FONTFLAG_OUTLINE);
}
...
POINT render::GetTextSize(DWORD font, const char* text)
{
	...
	POINT p; int x, y;
	g_csgo.m_surface->GetTextSize( font, wcstring, x, y );
	p.x = x;
	p.y = y;
	return p;
}
