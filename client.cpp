...
void Client::DrawHUD( ) {
	...
	const auto col_accent = Color( c_config::get( )->c[ "menu_color" ][ 0 ] * 255, c_config::get( )->c[ "menu_color" ][ 1 ] * 255, c_config::get( )->c[ "menu_color" ][ 2 ] * 255, 255 );
	...
	std::stringstream ss;
	ss << tfm::format( XOR( "spotlight.cc | fps: %i | latency: %i ms" ), fps, ms );

	render::FontSize_t size = render::hud.size( ss.str( ).c_str( ) );

	// background.
	render::rect_filled( m_width - size.m_width - 20, 10, size.m_width + 10, size.m_height + 2, col_background );

	// line watermark
	render::rect_filled( m_width - size.m_width - 20, 28, size.m_width + 12, 2, col_accent );
...
void Client::ClanTag()
{
	...
	if ( c_config::get()->b["clantag"] ) {
		...
		if (iPrevFrame != int(g_csgo.m_globals->m_curtime * 2.9) % 7) {
			switch (int(g_csgo.m_globals->m_curtime * 2.9) % 7) {
			case 0: {  SetClanTag("sp"); break; }
			case 1: {  SetClanTag("spot"); break; }
			case 2: {  SetClanTag("spotli"); break; }
			case 3: {  SetClanTag("spotligh"); break; }
			case 4: {  SetClanTag("spotlight"); break; }
			case 5: {  SetClanTag("spotlight."); break; }
			case 6: {  SetClanTag("spotlight.cc"); break; }

			default:;
			}
			iPrevFrame = int(g_csgo.m_globals->m_curtime * 2.9) % 7;
		}
...
void Client::Skybox()
{
	...
	if ( c_config::get( )->b[ "skybox_change" ] ) {
		switch ( c_config::get( )->i[ "skybox" ] ) {
		case 0: //Tibet
...
void Client::EndMove(CUserCmd* cmd) {
	...
	if ( c_config::get( )->i[ "config_mode" ] == 0 )
...
void Client::OnTick(CUserCmd* cmd) {
	...
	if ( c_config::get( )->b[ "ranks" ] && cmd->m_buttons & IN_SCORE ) {
...
void Client::SetAngles2(ang_t angle) {
	if (!g_cl.m_local || !g_cl.m_processing || !c_config::get( )->i[ "fake_yaw" ] )
		return;
...
void Client::print(const std::string text, ...) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf(colors::yellowgreen, XOR("[spotlight.cc] "));
...
void Client::UpdateRevolverCock() {
	...
	if (m_revolver_query == m_revolver_cock) {
		...
		// apply inattack.
		if ( c_config::get( )->i[ "config_mode" ] == 0 && m_revolver_query > m_revolver_cock)
